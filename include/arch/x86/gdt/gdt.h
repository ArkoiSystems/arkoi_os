...
// GDT selector indices for the predefined segments in the GDT
...
// Granularity flag bits for GDT entries
...
// Access flag bits for GDT entries
...
#define GDT_ACCESS_ACCESSED (1 << 0)                      // Set by the CPU when the segment is accessed

// Predefined GDT entry flags for common segment types (kernel/user code/data)
...
/**
 * @brief Represents an entry in the Global Descriptor Table (GDT) for x86 architecture.
 *
 * This structure defines the layout of a GDT entry, which includes the base address, segment limit, and various flags
 * that specify the properties of the memory segment. Each GDT entry describes a memory segment and its access rights,
 * which are used by the CPU for memory management and protection in protected mode. The base address is split into
 * three parts (low, middle, high) to fit within the 8-byte structure of a GDT entry, while the limit and flags are used
 * to define the size and access permissions of the segment.
 */
typedef struct {
    uint16_t segment_limit; /**< The limit of the segment (size) */
    uint16_t base_low;      /**< The lower 16 bits of the base address of the segment */
    uint8_t base_middle;    /**< The middle 8 bits of the base address of the segment */
    uint8_t access;         /**< The access flags for the segment (present, privilege level, type, etc.) */
    uint8_t granularity;    /**< The granularity and size flags for the segment (granularity, size, etc.) */
    uint8_t base_high;      /**< The higher 8 bits of the base address of the segment */
} __attribute__((packed)) gdt_entry_t;

/**
 * @brief Represents a pointer to the Global Descriptor Table (GDT) for x86 architecture.
 *
 * This structure defines the layout of the GDT pointer, which includes the limit (size) of the GDT and the base
 * address.
 */
typedef struct {
    uint16_t limit; /**< The limit of the GDT */
 ...
 *
 * Sets up and loads the processor's GDT entries required for protected-mode segment handling (such as kernel code/data
 * segments), preparing the CPU to use the configured segment descriptors.
 *
 ...
 *
 * Constructs a GDT entry with the specified base address, limit, and flags. GDT entries define memory segments and
 * their access properties in x86 architectures.
 *
 ...
 *
 * Initializes and returns a `gdt_ptr_t` value using the provided segment count and base address. This pointer is
 * typically used when loading the GDT (for example, with the `lgdt` instruction) during x86 setup.
 *
 ...
 *
 * This function loads the GDT pointer into the processor's GDTR register, making the specified GDT active for memory
 * segmentation and protection. This is typically called during kernel initialization to set up the memory segmentation
